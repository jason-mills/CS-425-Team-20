...
int main(int argc, char* argv[]) try
{
    ...
    std::string json_file_name = std::string(argv[3]) + "test.json";
    ...
    */
    ...
    for (int index = 0; index < rotations; index++)
    {
        ...
        for (int voting = 0; voting < scans_per_rotation; voting++)
        {
            ...
            save_vertices((std::string(argv[3]) + "out" + std::to_string(index)), points, index * degreesToRotate, 2);
            //std::cout << "Scanned! " << std::endl;
            count_lines(std::string(argv[3]) + "out" + std::to_string(index) + ".xyz");
        }
...
void write_metadata(int partitions, int scans, std::string path)
{
    ...
    std::ofstream output(path + "metadata.json");
